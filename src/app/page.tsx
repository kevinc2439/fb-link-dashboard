...
export default function Dashboard() {
  ...
  const handleGoogleAuth = async () => {
    try {
      // @ts-ignore - Bypassing strict type check for web/android build compatibility
  ...
  const handleCreateProfile = async (e: React.FormEvent) => {
    ...
    if (!user) return;
    try {
      await addDoc(collection(db, "fbProfiles"), {
        name: newProfile.name,
        loginHint: newProfile.loginHint,
        userId: user.uid,
        createdAt: new Date(),
      });
      setNewProfile({ name: "", loginHint: "" });
      setShowProfileModal(false);
    } catch (err: any) { console.error("Error adding profile:", err); }
  };
          ...
          <div className="bg-zinc-900 border border-zinc-800 p-10 rounded-[3rem] w-full max-w-md shadow-2xl relative">
            ...
            <form onSubmit={handleCreateProfile} className="space-y-6">
              <div className="space-y-2">
                ...
                <input 
                  placeholder="e.g. PRIMARY MCLUCK" 
              ...
              <div className="space-y-2">
                ...
                <input 
                  placeholder="e.g. operator_beta@gmail.com" 
                  value={newProfile.loginHint}
                  onChange={e => setNewProfile({...newProfile, loginHint: e.target.value})}
                  className="w-full bg-zinc-950 border border-zinc-800 p-5 rounded-2xl text-white outline-none focus:border-sky-500 transition-all font-bold" 
                />
              </div>
